...
export function useCommunityUser() {
  const [session, setSession] = useState<CommunitySession | null>(() =>
    readSession(),
  );
...
export function useIncrementShareCount() {
  ...
  return useMutation({
    mutationFn: async (postId: bigint) => {
      if (!actor) throw new Error("Actor not available");
      return actor.incrementShareCount(postId);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["communityPosts"] });
    },
  ...
  return useMutation({
    ...
    onSuccess: (_data, variables) => {
      queryClient.invalidateQueries({
        queryKey: ["userProfile", variables.userId.toString()],
      });
    },
  });
}
...
export function useChangeUserPassword() {
  ...
  return useMutation({
    mutationFn: async (data: {
      ...
      if (!actor) throw new Error("Actor not available");
      return actor.changeUserPassword(
        data.userId,
        data.oldPassword,
        data.newPassword,
      );
