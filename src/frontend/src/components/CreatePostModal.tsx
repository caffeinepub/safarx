...
import { type CommunitySession, useCreatePost } from "@/hooks/useQueries";
import { AlignLeft, Image, MapPin, PenSquare, X } from "lucide-react";
...
export default function CreatePostModal({ open, onClose, session }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const password = session.isGoogleUser
      ? "GOOGLE_AUTH"
      : (session.password ?? "");
    ...
    <Dialog open={open} onOpenChange={(v) => !v && handleClose()}>
      <DialogContent className="sm:max-w-lg bg-ivory-50 border-terracotta-200 p-0 overflow-hidden max-h-[90vh] overflow-y-auto">
        <DialogHeader className="sr-only">
          ...
          <DialogDescription>
            Create a new travel post to share with the community
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          ...
          <div className="space-y-1.5">
            <Label
              htmlFor="post-title"
              className="font-body text-terracotta-700 text-sm font-medium"
            >
          ...
          <div className="space-y-1.5">
            <Label
              htmlFor="post-body"
              className="font-body text-terracotta-700 text-sm font-medium flex items-center gap-1.5"
            >
