...
import { Eye, EyeOff, Lock, LogIn, User, UserPlus, X } from "lucide-react";
...
export default function CommunityAuthModal({
  ...
  const handleGoogleCredential = async (response: { credential: string }) => {
    ...
    try {
      ...
      if (!regResult.ok) {
        setGoogleError(
          regResult.message || "Registration failed. Please try again.",
        );
        setGoogleLoading(false);
        return;
      }
    ...
    <Dialog open={open} onOpenChange={(v) => !v && handleClose()}>
      <DialogContent className="sm:max-w-md bg-ivory-50 border-terracotta-200 p-0 overflow-hidden">
        <DialogHeader className="sr-only">
          ...
          <DialogDescription>
            Login or register to join the SafarX community
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="px-6 py-5 space-y-5">
          ...
          {GOOGLE_CLIENT_ID ? (
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-lg px-3 py-2.5 text-xs font-body text-amber-700">
              <strong>Google Sign-In:</strong> Set{" "}
              <code className="bg-amber-100 px-1 rounded">
                VITE_GOOGLE_CLIENT_ID
              </code>{" "}
              in your <code className="bg-amber-100 px-1 rounded">.env</code>{" "}
