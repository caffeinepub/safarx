...
export default function PackageCard({ pkg }: PackageCardProps) {
  const categoryColor =
    categoryColors[pkg.category] || "bg-saffron-100 text-saffron-700";
    ...
    <div className="group bg-card border border-border/50 rounded-2xl overflow-hidden shadow-card card-hover flex flex-col">
      ...
      <div className="relative h-48 overflow-hidden">
        ...
        <div className="absolute top-3 left-3">
          <span
            className={`inline-block px-2.5 py-1 rounded-full text-xs font-body font-semibold ${categoryColor}`}
          >
            {pkg.category}
          </span>
        </div>
      ...
      <div className="p-5 flex flex-col flex-1">
        ...
        >
          <button
            type="button"
            className="w-full flex items-center justify-center gap-2 bg-saffron-500 hover:bg-saffron-400 text-terracotta-900 font-body font-bold py-2.5 rounded-full transition-all text-sm group-hover:gap-3"
          >
            Enquire Now
