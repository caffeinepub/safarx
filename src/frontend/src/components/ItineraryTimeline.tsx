import type { Activity, GeneratedItinerary } from "@/utils/itineraryGenerator";
...
function ActivityCard({ activity }: { activity: Activity }) {
  ...
  return (
    <div className="flex gap-3 group">
      <div
        className={`flex-shrink-0 w-8 h-8 rounded-full border flex items-center justify-center mt-0.5 ${colorClass}`}
      >
      ...
      <div className="flex-1 pb-4">
        <div className="flex items-center gap-2 mb-1">
          <span
            className={`text-xs font-body font-semibold uppercase tracking-wider px-2 py-0.5 rounded-full border ${colorClass}`}
          >
            {activity.time}
          </span>
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        {itinerary.days.map((day) => (
          <div
            key={day.day}
            className="bg-card border border-border/60 rounded-2xl overflow-hidden shadow-card"
          >
            {/* Day Header */}
      ...
      <div className="bg-teal-800 rounded-2xl p-6 text-center text-ivory-100">
        ...
        <p className="font-body text-ivory-300 text-sm mb-4">
          Our travel experts can refine this itinerary and handle all bookings
          for you.
        </p>
        <a
          href="/contact"
